...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void instant_search() {
        ...
        Flux<String> suggestions =
            // my solution
            userSearchInput().windowUntilChanged(Function.identity(), (val1, val2) -> {
                return val2.startsWith(val1);
            }).flatMap(flux -> flux.last().flatMap(this::autoComplete))
                //todo: use one operator only
                ;

        // proposed solution
        suggestions = userSearchInput().switchMap(this::autoComplete);
