...
public class c5_CreatingSequence {
    ...
    public void interval() {
        Flux<Long> interval = Flux.range(0, Integer.MAX_VALUE)
            .map(Long::valueOf)
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Flux.generate(counter::get, (state, sink) -> {
        ...
        // OR
        repeated = Mono.fromCallable(counter::incrementAndGet)
            .repeat(9);
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            if (1 <= numbers.get() && numbers.get() <= 5) {
                sink.next(numbers.getAndIncrement());
            } else {
                sink.complete();
            }
        });
