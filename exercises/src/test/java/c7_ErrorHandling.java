...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void potato_potato() {
        Mono<String> currentUser = getCurrentUser().onErrorResume(e -> Mono.error(new SecurityException(e)));
    ...
    public void under_the_rug() {
        Flux<String> messages = messageNode().onErrorResume(e -> Flux.empty());
    ...
    public void error_reporter() {
        Flux<String> messages = messageNode().onErrorResume(e -> {
            return errorReportService(e).then(Mono.error(e));
        });
