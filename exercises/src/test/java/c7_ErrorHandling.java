...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void houston_we_have_a_problem() {
        ...
        Flux<String> heartBeat = probeHeartBeatSignal().timeout(Duration.ofSeconds(3))
            .onErrorResume(original -> {
                errorRef.set(original);
                return Flux.error(errorRef.get());
            });

        // OR
        heartBeat = probeHeartBeatSignal().timeout(Duration.ofSeconds(3))
    ...
    public void potato_potato() {
        Mono<String> currentUser = getCurrentUser().onErrorResume(original -> Mono.error(new SecurityException(original)));

        // OR
        currentUser = getCurrentUser().onErrorMap(SecurityException::new);
    ...
    public void have_a_backup() {
        Flux<String> messages = messageNode().onErrorResume(error -> backupMessageNode());
    ...
    public void error_reporter() {
        Flux<String> messages = messageNode().onErrorResume(error -> errorReportService(error).then(Mono.error(error)));
    ...
    public void unit_of_work() {
        Flux<Task> taskFlux = taskQueue().flatMap(task -> {
            return task.execute()
                .then(task.commit())
                .onErrorResume(error -> task.rollback(error))
                .thenReturn(task);
        });
    ...
    public void resilience() {
        Flux<String> content = getFilesContent()
            .flatMap(element -> {
                return element.doOnError(System.err::println)
    ...
    public void back_off() {
        Mono<String> connection_result = establishConnection().retryWhen(Retry.backoff(3, Duration.ofSeconds(5)));
    ...
    public void good_old_polling() {
        Flux<String> alerts = nodeAlerts().repeatWhenEmpty(it -> it.delayElements(Duration.ofSeconds(1)))
            .repeat()
            .take(2);
